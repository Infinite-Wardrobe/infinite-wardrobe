...
def get_top_colors(color_pixels, color_codes):
    ...
    top_colors = sorted(data, key=lambda x: x['Pixels'], reverse=True)[:3]
    response_data = [{"color": color["color"], "simple_color": get_simple_color_name(color["rgb"]) , "rgb": color["rgb"]} for color in top_colors]
